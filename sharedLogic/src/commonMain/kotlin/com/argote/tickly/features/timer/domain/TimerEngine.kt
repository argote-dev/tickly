...
class TimerEngine(
        ...
        nextPhase =
            when (phase) {
                TimerPhase.FOCUS -> {
                    ...
                    val breakPhase =
                        if (completedFocusBlocks >= settings.blocksUntilLongBreak) {
    ...
    fun skip(nowMillis: Long) {
        ...
        when (phase) {
            TimerPhase.FOCUS -> {
                prepare(TimerPhase.SHORT_BREAK)
            }
    ...
    fun serialize(): String = listOf(
        "tickly",
        "1",
        "fm=${settings.focusMinutes}",
        "sm=${settings.shortBreakMinutes}",
        "lm=${settings.longBreakMinutes}",
        "lb=${settings.blocksUntilLongBreak}",
        "ai=${settings.accentIndex}",
        "si=${settings.soundIndex}",
        "v=${if (settings.vibrationEnabled) 1 else 0}",
        "ab=${if (settings.animatedBackground) 1 else 0}",
        "ks=${if (settings.keepScreenOn) 1 else 0}",
        "lang=${settings.language}",
        "p=${phase.name}",
        "s=${status.name}",
        "cb=$completedFocusBlocks",
        "d=$intervalDurationMillis",
        "r=$remainingAtRestMillis",
        "dl=$deadlineMillis",
        "n=${nextPhase.name}",
    ).joinToString("|")
    ...
    private fun restore(snapshot: String) {
        ...
        try {
            val restoredSettings =
                TimerSettings(
                    focusMinutes = map.int("fm"),
                    shortBreakMinutes = map.int("sm"),
                    longBreakMinutes = map.int("lm"),
                    blocksUntilLongBreak = map.int("lb"),
                    accentIndex = map.int("ai"),
                    soundIndex = map.int("si"),
                    vibrationEnabled = map.bool("v"),
                    animatedBackground = map.bool("ab"),
                    keepScreenOn = map.bool("ks"),
                    language = map.required("lang"),
                ).validated()
